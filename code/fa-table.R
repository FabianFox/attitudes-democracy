# Function created by Francisco Wilhelm
# blog: https://www.franciscowilhelm.com/post/exploratory-factor-analysis-table/
# source: https://github.com/franciscowilhelm/r-collection/blob/master/fa_table.R

# Slightly adjusted for our project
...
fa_table <- function(x, varlabels = NULL, title = "Factor analysis results", diffuse = .10, small = .30, cross = .20, sort = TRUE) {
  ...
  ind_table <- dplyr::tibble(varlabels, loadings) %>%
    ...
    dplyr::mutate(across(c(Communality), round, 2))
